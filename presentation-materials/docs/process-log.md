...
- 발표 스크린샷과 별도로, 서비스 구조와 차별점을 설명하는 16:9 보조 이미지 8장을 `visuals/` 폴더에 추가했다.
- 이미지 생성은 `scripts/generate_presentation_visuals.py`로 자동화했다. 발표 문구를 수정해야 할 때 같은 스타일로 다시 생성할 수 있다.
- 추가한 이미지 주제는 서비스 포지셔닝, 분석 파이프라인, 생성형 AI와의 차별점, 신뢰성 설계, 출시 로드맵, 로컬 처리, 발표 요약, 심사위원 Q&A 대응 맵이다.
- `docs/visual-asset-guide.md`에 각 이미지의 발표 역할과 추천 사용 위치를 문서화했다.
- 생성 후 주요 이미지를 직접 열어 글자 겹침, 박스 겹침, 연결선 겹침 여부를 확인했고, `04-trust-loop.png`와 `08-judge-qna-map.png`는 배치 문제를 수정한 뒤 다시 생성했다.

## 9. 기존 스크린샷 사용 가이드 추가

날짜: 2026-05-21

- `screenshots/` 폴더에 있던 기존 발표용 스크린샷 8장의 사용 목적을 `docs/screenshot-usage-guide.md`로 문서화했다.
- 기본 서비스 캡처(`images/`), 발표 핵심 장면(`screenshots/`), 개념 설명 이미지(`visuals/`)의 차이를 구분해 발표자가 자료를 고르기 쉽게 정리했다.
- 신뢰성, 위험 조건, 체크리스트, 히스토리, 자동화, SQLite 구조처럼 심사 질문과 연결되는 포인트를 이미지별로 정리했다.
