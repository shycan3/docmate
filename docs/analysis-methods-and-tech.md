...
심사 문답에서 가장 먼저 방어해야 할 지점은 "모든 PDF를 요약할 수 있는가"이다. 현재 답은 명확히 아니다. DocMate는 범용 PDF 요약기가 아니라, 장학금·청년정책 공고처럼 반복되는 항목 구조를 가진 문서를 신청 준비 데이터로 바꾸는 도구이다.

지원 가능성이 높은 문서:

- 텍스트 추출이 가능한 PDF 또는 TXT
- `신청 기간`, `지원 대상`, `지원 내용`, `제출 서류`, `신청 방법` 같은 라벨이 있는 공고
- 나이, 거주지, 소득구간, 재학 여부처럼 조건 표현이 명확한 공고
- 제출 서류와 신청 URL이 본문에 드러나는 공고

부분적으로 가능한 문서:

- 라벨은 다르지만 유사한 항목 순서가 있는 문서
- 일부 조건만 명확하고 나머지는 본문 서술에 섞여 있는 문서
- 표나 목록이 섞여 있지만 텍스트 추출은 가능한 문서

현재 한계가 큰 문서:

- 스캔 이미지 PDF 또는 OCR이 필요한 문서
- 항목 구분이 거의 없는 자유서술형 보고서
- 표, 각주, 이미지, 본문이 복잡하게 섞여 사람이 문맥으로 해석해야 하는 문서
- 사업별 세부요건이 여러 하위 문서에 흩어져 있는 공고

따라서 발표에서는 "모든 문서를 요약한다"가 아니라 "구조화된 정책·장학금 공고를 신청 준비 흐름으로 바꾼다"라고 설명해야 한다.
...
현재 데모는 샘플 공고와 단위 테스트 중심으로 검증되어 있다. 실제 서비스 신뢰도를 주장하려면 다음 평가가 추가되어야 한다.

| 평가 항목 | 측정 방법 | 목표 |
| --- | --- | --- |
| 항목 추출 정확도 | 실공고 fixture에서 신청 기간, 지원 대상, 제출 서류, URL이 맞게 추출되는지 확인 | 항목별 precision/recall 기록 |
| 자격 판정 안전성 | 정답 프로필 세트를 만들어 `eligible`, `needs_review`, `ineligible` 결과 비교 | 애매한 경우 `needs_review` 우선 |
| 위험 조건 탐지 | 중복지원, 자동 탈락, 서류 미비 문구가 있는 공고와 없는 공고를 분리 테스트 | false negative 최소화 |
| 원문 근거 품질 | 결과 근거가 실제 문서 문장과 연결되는지 사람이 검수 | 판정 근거 누락 최소화 |
| 사용자 효용 | 실제 사용자에게 같은 공고를 수동 확인하게 한 뒤 시간과 확신도 비교 | 시간 절감과 누락 감소 측정 |

발표에서는 "현재 정확도 수치를 과장하지 않고, 실공고 fixture와 사용자 연구를 다음 검증 단계로 제시한다"가 가장 안전하다.
